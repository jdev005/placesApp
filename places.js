...
/* the function above checks for geoloaction support 
geolocation is the property in the navigotor object for geolocation support 
if there is support it calls the getCurrentPosition method of the navigator.gelocation object 
which does the work of getting the browsers location , it has handler function, here named displayLocation*/
...
/* when the displayLocation is called the geolocation api passes it a position object 
that contains information about the browsers location 
we set the lat and long values and then call the show map function */

function showMap(coords){
	// we need to make a map object by using google's map constructor
	  googleLatAndLong= new google.maps.LatLng(coords.latitude,coords.longitude);

	//lets add some map options provided by google 
...
function callmap(){
	...
	/* ^ another constructor from google that takes and element and options and creates and returns a map object */
...
function callback(results,status){
	...
	and a status as aruments, what this does is if the status is OK
	for every result we have found it adds a marker on the map */
